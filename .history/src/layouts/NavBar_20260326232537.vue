...
const cacheMmsi = async (mmsi) => {
  try {
    const res = await axios.get('http://localhost:8877/api/ship/cacheMmsi', {
      params: { mmsi },
      timeout: 10000
    })
    ...
    emit('mmsi-cached', { mmsi, response: res.data })
    return { success: true, data: res.data }
  } catch (err) {
    console.error('❌ 缓存 MMSI 失败:', err)
    
    // 即使缓存失败，也允许继续查询（后端可能有其他机制）
    localStorage.setItem('currentMmsi', mmsi)
...
const handleSearch = async () => {
  ...
  try {
    const result = await cacheMmsi(mmsi)
    
    if (result.success) {
      showStatus(`✅ MMSI ${mmsi} 缓存成功`, 'success')
      // 跳转到轨迹回放页，携带 MMSI 参数
      router.push({ 
        path: '/track_flow', 
        query: { 
          mmsi,
          t: Date.now() // 添加时间戳避免路由缓存
        } 
      })
    } else {
      showStatus(`⚠️ 缓存失败，但仍可查询`, 'warning')
      router.push({ 
        path: '/track_flow', 
        query: { mmsi, t: Date.now() } 
      })
    }
  } catch (err) {
    console.error('搜索处理异常:', err)
    showStatus('搜索异常，请重试', 'error')
  } finally {
