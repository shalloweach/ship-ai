...
const handleSearch = async () => {
  ...
  try {
    const response = await shipApi.cacheMmsi(mmsi)
    
    // ✅ 后端返回的是字符串，如：
    // "Successfully cached MMSI 636019894 with 12456 records"
    // 或 "Failed to cache MMSI 636019894"
    ...
    // 匹配 "with X records" 模式
    const match = response.match(/with (\d+) records/)
    if (match) {
