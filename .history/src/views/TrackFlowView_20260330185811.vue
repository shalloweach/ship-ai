...
<template>
  <main class="track-flow-container" role="main" aria-label="船舶轨迹查询系统">
    ...
    <aside class="side-panel" role="complementary" aria-label="查询控制面板">
      ...
      <!-- 模块 1: 时间/索引查询 -->
  <TimeSliderPanel
    :key="`time-${mmsi}`"  <!-- 🔑 添加 key，mmsi 变化时强制重建 -->
    ref="timeSliderRef"
    :mmsi="mmsi"
...
watch(mmsi, async (newMmsi, oldMmsi) => {
  if (newMmsi && newMmsi !== oldMmsi) {
    ...
    // 初始化地图
    await nextTick()
    if (timeSliderRef.value?.resetToDefault) {
      timeSliderRef.value.resetToDefault()
    }

    // 🔹 3. 清空地图轨迹（如果 mapOp 有对应方法）
    // if (mapOp?.clearTrajectory) {
    //   mapOp.clearTrajectory()
    // }

    // // 🔹 4. 可选：触发新数据加载
    // await timeSliderRef.value?.emitQuery?.()
