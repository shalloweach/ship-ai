  ...
  <main class="track-flow-container" role="main" aria-label="船舶轨迹查询系统">
      ...
      <div class="panel-content-wrapper">
        ...
        <transition name="panel-fade" mode="out-in">
          <div
            ...
            id="marker-panel" role="tabpanel"
            class="panel-content" key="marker"
          >
            <MarkCard
              ref="markCardRef"
              :mmsi="mmsi"
              :marks="pendingMarks"
              :mapOp="mapOp"
              @mark-select="onMarkSelect"      <!-- 点击记录跳转地图 -->
              @mark-reselect="onMarkReselect"  <!-- 重新选择起止点 -->
              @mark-update="onMarkUpdate"      <!-- 更新标记字段 -->
              @mark-delete="onMarkDelete"      <!-- 删除标记 -->
              @submit="onSubmitMarks"          <!-- 提交所有标记 -->
            />
          </div>
        </transition>
      </div>
    ...
    <section class="map-wrapper" role="region" aria-label="轨迹地图展示">
      ...
      <div v-if="tempHighlight?.length > 0" class="temp-highlight-info">
        <span>✨ 已选择 {{ tempHighlight.length }} 个轨迹点</span>
      </div>
    ...
    <Teleport to="body">
      <div v-if="editDialog.visible" class="mark-dialog-overlay" @click.self="closeEditDialog">
        <div class="mark-dialog-container" ref="dialogContainer">
          <!-- 内容通过 innerHTML 注入，事件通过全局回调处理 -->
          <div v-html="editDialog.content"></div>
        </div>
      </div>
    </Teleport>
