...
onMounted(async () => {
  try {
    ...
    console.log('✅ 地图实例化成功:', mapOp.map)
  
    handleUrlQuery(route.query)
...
const handleUrlQuery = (query) => {
  ...
  // 🔄 通知子组件重置/加载数据
  // 使用 nextTick 确保DOM和props已更新
  setTimeout(() => {
    timeSliderRef.value?.reset?.()
    playerCardRef.value?.reset?.()
  }, 0)
  
  console.log(`🚢 加载船舶: MMSI=${newMmsi}, 点数=${newTotalCount}`)
}

// ========== 🔁 监听后续URL变化（用户手动修改地址栏时） ==========
watch(
  () => route.query,
  (newQuery, oldQuery) => {
    // 跳过初始化时的首次调用（onMounted已处理）
    if (!oldQuery || Object.keys(oldQuery).length === 0) return
    
    // 只有参数真正变化时才处理
    if (newQuery.mmsi !== oldQuery.mmsi || newQuery.totalCount !== oldQuery.totalCount) {
      handleUrlQuery(newQuery)
    }
  },
  { deep: true }
)
