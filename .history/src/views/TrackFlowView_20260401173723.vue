...
const onResize = (e) => {
  ...
  // ✅ 限制范围（与 CSS min/max 保持一致）
  newWidth = Math.min(800, Math.max(320, newWidth))
  
  panelWidth.value = newWidth
  
  // ✅ 如果仍需用 CSS 变量（如 tooltip 定位），确保设置正确
...
/* ============ ✨ 修复：拖拽手柄（回归文档流） ============ */
.resize-handle {
  /* ✅ 移除 position: fixed，使用正常流 + flex-shrink */
  width: 5px;
  min-width: 5px;  /* ✅ 防止被压缩 */
  cursor: ew-resize;
  background-color: transparent;
  transition: background-color 0.2s;
  flex-shrink: 0;  /* ✅ 关键：防止 flex 布局时手柄被压缩 */
  z-index: 30;
  user-select: none;
  /* ✅ 可选：添加视觉反馈 */
...
body.resizing .map-wrapper {
  pointer-events: auto; /* ✅ 但手柄和地图区域仍需响应事件 */
}
