...
watch(
  ...
  (newQuery) => {
    ...
    // 参数真正变化时才更新
    const isMmsiChanged = newMmsi && newMmsi !== mmsi.value
    const isCountChanged = newTotalCount !== totalCount.value
    
    if (isMmsiChanged || isCountChanged) {
      console.log('URL 参数更新:', { oldMmsi: mmsi.value, newMmsi, oldCount: totalCount.value, newCount: newTotalCount })
      mmsi.value = newMmsi
      totalCount.value = newTotalCount
      
      // 更新 hasData 状态
      hasData.value = !!newMmsi && newTotalCount > 0
    }
  },
...
// ========== 📡 监听 mmsi 变化，重置子组件 ==========
watch(mmsi, async (newMmsi, oldMmsi) => {
  if (newMmsi && newMmsi !== oldMmsi) {
    console.log('mmsi 变化，重置子组件，地图实例保持不变')
    
    // 等待 DOM 更新完成
    await nextTick()
    
    // 重置时间滑块和播放器（地图实例保持不变）
    if (timeSliderRef.value) {
      console.log('重置 TimeSliderPanel')
      timeSliderRef.value.reset?.()
    }
    if (playerCardRef.value) {
      console.log('重置 PlayerCard')
...
onMounted(async () => {
  try {
    ...
    // 如果子组件已经存在，它们会自动通过 provide/inject 获取到地图实例
    console.log('地图实例已通过 provide 提供给所有子组件')
...
const clearMap = () => {
  console.log('清空地图')
  if (mapOp && typeof mapOp.clear === 'function') {
    mapOp.clear()
  }
}
...
const centerMap = () => {
  console.log('居中显示地图')
  if (mapOp && typeof mapOp.center === 'function') {
    mapOp.center([31.2, 122], 8)
  }
}

const refreshQuery = () => {
  console.log('重新查询')
  if (timeSliderRef.value && typeof timeSliderRef.value.emitQuery === 'function') {
