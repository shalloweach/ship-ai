...
// ========== 📦 状态管理 ==========
// ✅ 方案：优先使用 inject 获取 NavBar 传递的数据，降级使用路由参数
const injectedShipInfo = inject('shipSearchInfo', null)

const mmsi = ref('')
const totalCount = ref(0)
const currentPointsCount = ref(0)

// ✅ 同步 inject 或路由参数到本地状态
const syncShipInfo = (source) => {
  const newMmsi = source?.mmsi || source?.value?.mmsi || ''
  const newTotalCount = Number(source?.totalCount || source?.value?.totalCount || 0)
  
  if (newMmsi && newMmsi !== mmsi.value) {
    mmsi.value = newMmsi
...
onMounted(async () => {
  ...
  // ✅ 降级：从路由参数获取（immediate: true 会立即执行一次）
  // watch 会在下方触发
})

// ✅ 监听 inject 响应式变化（如果父组件更新）
if (injectedShipInfo) {
  watch(
    () => injectedShipInfo.value,  // 注意：inject 的是 ref，所以用 .value
    (newInfo) => {
      if (syncShipInfo({ value: newInfo })) {
        loadTrajectory()
      }
    },
    { deep: true }
