...
<template>
  <main class="track-flow-container" role="main" aria-label="船舶轨迹查询系统">
    ...
    <!-- 🗺️ 右侧地图区域 -->
    <section class="map-area" @click="onMapClick">
      <div :id="`map-${mmsi}`" ref="mapContainerRef" class="map-container"></div>
      ...
      <!-- 信息浮层 -->
      <div v-if="trackFlow.player.hasData.value && !loading" class="map-info">
        <span>🚢 {{ mmsi }}</span>
        <span>|</span>
        <span>{{ trackFlow.player.allPoints.value.length }} 点</span>
        <span v-if="trackFlow.player.isPlaying.value">|</span>
        <span v-if="trackFlow.player.isPlaying.value">▶️ 播放中</span>
      </div>
    </section>
...
// 父组件中查询总点数（示例）
const fetchTotalPoints = async (mmsi) => {
  try {
    // 调用您的后端接口获取该 MMSI 的总点数
    const res = await axios.get('http://localhost:8877/api/ship/count', {
      params: { mmsi }
    })
    totalPoints.value = res.data?.count || 0
  } catch (err) {
    console.warn('获取总点数失败:', err)
    totalPoints.value = null  // 降级使用默认值
  }
}
