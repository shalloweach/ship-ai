  ...
  <main class="track-flow-container" role="main" aria-label="船舶轨迹查询系统">
    ...
    <aside class="side-panel" role="complementary" aria-label="查询控制面板">
      ...
      <TimeSliderPanel
        ref="timeSliderRef"
        :mmsi="currentMmsi"
        :totalCount="shipTotalCount"  <!-- ✅ 新增：传递轨迹总数 -->
...
// ✅ 当前 MMSI：优先使用路由参数，其次使用搜索注入
const currentMmsi = ref(route.query.mmsi || '')
// ✅ 轨迹总数：来自搜索结果
const shipTotalCount = ref(Number(route.query.totalCount) || 0)
...
watch(
  ...
  (newInfo) => {
    if (newInfo?.mmsi && newInfo?.totalCount > 0) {
      ...
      // 自动加载轨迹
      loadTrajectory(newInfo.mmsi)
    }
  },
  { deep: true }
)

// ✅ 初始加载：仅当路由带参数时加载，否则显示空状态
onMounted(async () => {
  await initDeps()
  
  // ✅ 初始状态：不自动查询，等待用户搜索或路由参数
...
// ✅ 加载轨迹核心函数（保持不变，仅使用 shipTotalCount 作为参考）
const loadTrajectory = async (mmsi) => {
  if (!mmsi || !api?.fetchTrajectoryByTime) return
  ...
  try {
    ...
    if (result?.success) {
      ...
      totalPoints.value = shipTotalCount.value > 0 
        ? shipTotalCount.value 
        : await estimateTotalPoints({ mmsi, time: { start, end } })
