 ...
 * 加载轨迹数据
 * 主要流程：
 * 1. 重置 TimeSlider 到默认状态
 * 2. TimeSlider 内部会自动加载数据
 * 3. 通过 @points-updated 事件获取点数量
 */
const loadTrajectory = async () => {
  ...
  try {
    ...
    if (!timeSlider) {
      throw new Error('TimeSlider 组件未初始化')
    }
    
    // 重置 TimeSlider 到默认状态（这会触发内部数据加载）
    await timeSlider.resetToDefault()
    
    // 注意：currentPointsCount 会通过 @points-updated 事件更新
    // 不需要在这里手动设置
...
/**
 * 处理 TimeSlider 点数量更新事件
 */
const handlePointsUpdated = (count) => {
 ...
 */
const clearMap = () => {
  if (mapOp.value?.clearLayers) {
    mapOp.value.clearLayers()
  }
}
 ...
 */
const centerMap = () => {
  if (mapOp.value?.fitBounds) {
    mapOp.value.fitBounds()
  }
}
...
const handlePlayerPlay = () => {
  ...
  // 调用 TimeSlider 的播放方法
  if (timeSliderRef.value?.play) {
    timeSliderRef.value.play()
  }
}
...
const handlePlayerPause = () => {
  ...
  // 调用 TimeSlider 的暂停方法
  if (timeSliderRef.value?.pause) {
    timeSliderRef.value.pause()
  }
}
...
/**
 * 监听路由参数变化
 * 当 URL 中 mmsi 或 totalCount 改变时，重新加载数据
 */
watch(
  () => route.query,
  (newQuery) => {
...
onUnmounted(() => {
  // 清理资源
  if (timeSliderRef.value?.destroy) {
    timeSliderRef.value.destroy()
  }
  if (mapOp.value?.destroy) {
    mapOp.value.destroy()
  }
})
