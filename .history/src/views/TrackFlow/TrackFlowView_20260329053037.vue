...
<script setup>
import { ref, computed, watch, inject, onMounted } from 'vue'  // ✅ 添加 watch 和 computed
import TimeSliderPanel from '@/components/TimeSliderPanel/TimeSliderPanel.vue'
import PlayerCard from '@/components/TimeSliderPanel/TimeSliderPanel.vue'
...
const playerIsPlaying = computed(() => player.value?.isPlaying?.value ?? false)
...
// 初始化
onMounted(async () => {
  await mapOp.init({ center: [121.5, 31.2], zoom: 12 })  // ✅ 修正坐标顺序 [lng, lat]
  // ✅ 修正 useMap 调用，传入正确的参数
  if (typeof useMap === 'function') {
    useMap(mapOp, { mmsi: currentMmsi, totalCount })
  }
})

// ✅ 补充缺失的 loadTrajectory 函数
const loadTrajectory = async () => {
  if (!currentMmsi.value || totalCount.value <= 0) return
  loading.value = true
  error.value = null
  try {
    // TODO: 替换为实际的业务逻辑，例如调用 API 获取轨迹数据
    // const res = await api.getTrajectory({ mmsi: currentMmsi.value, ... })
    // currentPointsCount.value = res.data?.length ?? 0
    hasData.value = true
  } catch (e) {
    error.value = e.message || '轨迹加载失败'
    hasData.value = false
  } finally {
    loading.value = false
  }
...
const exportTrajectory = () => {
  // TODO: 实现导出逻辑
  console.log('导出轨迹:', currentMmsi.value)
}
const clearMap = () => {
  mapOp?.clearLayers?.()
  hasData.value = false
...
const centerMap = () => {
  if (currentMmsi.value && hasData.value) {
    mapOp?.fitTrajectory?.()
  }
}
const refreshQuery = () => {
