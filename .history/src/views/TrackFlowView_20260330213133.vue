  ...
  <main class="track-flow-container" role="main" aria-label="船舶轨迹查询系统">
    ...
    <section class="map-wrapper" role="region" aria-label="轨迹地图展示" @click="onMapClick">
      ...
      <div class="config-item config-item-full">
        <label class="config-label">底图图层</label>
        <div class="layer-switcher">
          <button
            v-for="layer in controller.getAvailableLayers?.() || []"
            :key="layer.id"
            class="layer-btn"
            :class="{ active: controller.localLayerId === layer.id }"
            @click="controller.onLayerChange(layer.id)"
            :title="layer.label"
          >
