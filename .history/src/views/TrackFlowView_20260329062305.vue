<!-- src/viewsTrackFlowContainer.vue -->
<template>
...
watch(
  ...
  (newQuery) => {
    ...
    if (isMmsiChanged || isCountChanged) {
      console.log('URL 参数更新:', { 
        oldMmsi: mmsi.value, 
        newMmsi, 
        oldCount: totalCount.value, 
...
watch(mmsi, async (newMmsi, oldMmsi) => {
  if (newMmsi && newMmsi !== oldMmsi) {
    ...
    // 加载船舶轨迹数据
    await loadShipTrajectory(newMmsi)
  }
})

// 加载船舶轨迹数据
const loadShipTrajectory = async (shipMmsi) => {
  try {
    // 这里调用你的 API 获取轨迹数据
    // const trajectoryData = await fetchTrajectory(shipMmsi)
    // mapOp.drawTrajectory(trajectoryData)
    console.log('加载船舶轨迹:', shipMmsi)
  } catch (error) {
...
// ========== 🗺️ 地图初始化 ==========
onMounted(async () => {
  // 初始化地图：上海坐标 [121.5, 31.2]，缩放级别 12
  await mapOp.init({ center: [31.2, 122], zoom: 8 })
  
  // 如果 URL 中已有 mmsi 参数，自动加载
  if (mmsi.value) {
    await loadShipTrajectory(mmsi.value)
  }
})
...
defineExpose({
  ...
  centerMap: () => {
    mapOp.center?.([31.2, 121.5], 12)
  },
  loadTrajectory: loadShipTrajectory,
  currentPointsCount: computed(() => totalCount.value)
})
</script>
