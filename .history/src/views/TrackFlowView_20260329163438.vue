...
<script setup>
import { ref, provide, watch, onMounted, shallowRef, markRaw, nextTick } from 'vue'
...
// 🗺️ 地图操作 - 关键修复：确保 provide 的是响应式容器
const mapOp = shallowRef(null)
// ✅ 立即赋值，避免 provide 时值为空
mapOp.value = markRaw(useMapOperation(mapContainerRef))
// ✅ 提供 ref 本身，让子组件能通过 .value 响应式获取最新值
provide('mapOp', mapOp)
...
onMounted(async () => {
  ...
  // ✅ 可选：通知子组件 map 已就绪（如果需要）
  // timeSliderRef.value?.onMapReady?.()
})
...
watch(
  () => route.query,
  (newQuery) => {
    mmsi.value = newQuery.mmsi || ''
    totalCount.value = Number(newQuery.totalCount) || 0
