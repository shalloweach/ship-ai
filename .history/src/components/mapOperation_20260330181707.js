...
export default function useMapOperation(containerRef) {
   ...
   * @param {[number, number]} options.center - 中心点 [纬度, 经度] ⚠️注意顺序
   * @param {number} options.zoom - 初始缩放级别 (3-20)
  ...
  const init = async (options = {}) => {
    ...
    // 2. ⚠️ 坐标转换：高德使用 [经度, 纬度]，业务层传入 [纬度, 经度]
  ...
  const ensureAMapLoaded = () => {
    return new Promise((resolve, reject) => {
      ...
      script.src = `https://webapi.amap.com/maps?v=2.0&key=9a74bdb399a18e1dbe15f217c72ca022&plugin=AMap.Scale,AMap.ToolBar`
      script.async = true
      script.onload = () => {
        // 等待 AMap 初始化
        const check = setInterval(() => {
  ...
  const parsePointToLngLat = (point) => {
    ...
    // 格式 1: { lng, lat } 或 { longitude, latitude }
    ...
    // 格式 2: [lng, lat] 或 [lat, lng] - 通过数值范围判断
