...
import { useTrajectoryPopup } from './useTrajectoryPopup'
      ...
      if (path.length <= 5000) {
        pointMarkers = path.map((lnglat, idx) => {
          ...
          marker.on('click', (e) => {
            ...
            showPopup(marker, point, {
              mmsi: mmsi.value,
              // 回调函数
              onCopy: (coord, pt) => {
                console.log('📋 已复制坐标:', coord)
                // 可添加 toast 提示
              },
              onTimelineJump: (pointIdx) => {
                // 通知时间轴组件跳转
                timeSliderRef.value?.jumpToPoint?.(pointIdx)
              },
              onShowDetail: (pt) => {
                // 打开详情模态框
                showDetailModal(pt)
              ...
              onFilterByStatus: (status) => {
                // 筛选相同状态的轨迹点
                highlightPointsByStatus(status)
              }
            })
          })
