  ...
  <footer class="time-slider">
    <div class="slider-container">
      ...
      <div class="slider-track">
        ...
        <div
          class="slider-range"
          :style="{ left: (startDayIndex / (totalDays - 1)) * 100 + '%', width: ((endDayIndex - startDayIndex) / (totalDays - 1)) * 100 + '%' }"
        ></div>
      </div>
...
// 当前日期
const currentStartDate = computed(() => dayjs(`${props.year}-01-01`).add(startDayIndex.value, 'day'))
const currentEndDate = computed(() => dayjs(`${props.year}-01-01`).add(endDayIndex.value, 'day'))
...
const monthPositions = computed(() => {
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    months.push({ name: `${m}月`, left: (startOfMonth / (totalDays.value - 1)) * 100 })
  }
  return months
})
