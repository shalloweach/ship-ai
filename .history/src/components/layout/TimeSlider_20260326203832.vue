  ...
  <footer class="time-slider">
    <div class="slider-container">
      ...
      <div class="month-labels">
        <span v-for="(label, idx) in monthPositions" :key="idx" :style="{ left: label.left + '%' }">
          {{ label.name }}
        </span>
      </div>
    </div>
...
// 当天日期
const currentDate = computed(() => dayjs(`${props.year}-01-01`).add(dayIndex.value, 'day'))
...
// Watch year prop，重置滑块
watch(() => props.year, () => dayIndex.value = 0)
