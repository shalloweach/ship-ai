    ...
    >
      ...
      <div class="mode-switch" role="toolbar" aria-label="查询模式">
        ...
        <span class="range-hint" aria-live="polite">
          全局范围: {{ formatRange(0) }} ~ {{ formatRange(props.totalCount) }} 
          (共 {{ props.totalCount + 1 }} 点)
...
// 解构 useTimeSlider 返回的所有响应式数据和方法
const {
  // 状态
  startTimestamp,
  endTimestamp,
...
watch(
  ...
  ([newStartTs, newEndTs, newStartIdx, newEndIdx]) => {
    if (newStartTs != null && newStartTs !== Math.floor(startTimestamp.value / 1000)) {
      startTimestamp.value = newStartTs * 1000
    }
    if (newEndTs != null && newEndTs !== Math.floor(endTimestamp.value / 1000)) {
      endTimestamp.value = newEndTs * 1000
    }
