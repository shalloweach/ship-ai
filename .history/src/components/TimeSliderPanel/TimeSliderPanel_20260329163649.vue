...
// ✅ 使用 computed 确保响应式获取 map 实例
const mapOp = computed(() => mapOpRef?.value)
const mapInstance = computed(() => mapOp.value?.map)

// ✅ 修复：不要在 setup 同步阶段访问 map（此时 init() 未执行）
// ❌ 错误写法：
// console.log('🗺️ 内部 map 实例:', mapOp.value?.map)  // → null

// ✅ 正确写法1: onMounted 中访问
onMounted(() => {
  console.log('🗺️ 内部 map 实例:', mapInstance.value)
  
  // ✅ 如果仍为 null，说明父组件 init() 未完成，用 watch 监听
  if (!mapInstance.value) {
    console.warn('⚠️ map 未就绪，等待父组件 init()...')
  }
})
...
const {
  ...
  updateTime,
  updateIndex,
  // ✅ 传入 computed 或普通对象，根据 useTimeSlider 的实现调整
  // 如果 useTimeSlider 需要响应式，传入 mapOp（computed）
  // 如果需要普通对象，传入 mapOp.value
