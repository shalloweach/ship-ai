...
export function useTimeSlider(mapOp, mmsi, totalCount) {
  ...
  const resetToDefault = () => {
    const firstShowPoints = Math.min(1000, shipSearchInfo.totalCount + 1)
    
    try {
      const result = await getTrajectoryByIndex(mmsi, start, firstShowPoints)
  ...
  const expandFull = () => {
    // 时间扩展为全局范围
    if (props.globalMinTs != null && totalCount != null) {
      startTimestamp.value = toMilliseconds(0)
      endTimestamp.value = toMilliseconds(totalCount)
    }
