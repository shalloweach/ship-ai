...
const props = defineProps({
  ...
  mapOp: { type: Object, default: null },  // 保留作为 props，同时也会通过 inject 获取
  // 外部控制参数（可选）
...
// ========== 📦 使用 inject 获取父组件提供的地图实例 ==========
const injectedMapOp = inject('mapOp', null)
const injectedIsMapReady = inject('isMapReady', null)

// 优先使用 inject 的地图实例，如果没有则使用 props 的
const finalMapOp = computed(() => injectedMapOp || props.mapOp)

// 监听地图就绪状态
watch(
  () => injectedIsMapReady?.value,
  (ready) => {
    if (ready && finalMapOp.value) {
      console.log('TimeSliderPanel: 地图已就绪，可以开始操作')
      // 可以在这里执行需要地图的操作
    }
  },
  { immediate: true }
...
watch(
  () => props.mmsi,
  (val) => {
    console.log('TimeSliderPanel 最新 mmsi:', val)
    // 更新内部状态 or 请求数据
  },
  { immediate: true }
)

// 使用 useTimeSlider，传入最终的地图实例
const {
  startTimestamp,
  endTimestamp,
  startIndex,
  ...
  globalMaxIdx,
  lastModifiedSource
} = useTimeSlider(finalMapOp, props.mmsi, props.totalCount)
...
// 重置方法（供父组件调用）
const reset = () => {
  console.log('TimeSliderPanel reset 被调用，重置组件状态')
  // 重置为默认值
  resetToDefault()
}
...
onMounted(() => {
  if (finalMapOp.value) {
    console.log('TimeSliderPanel 已获取到地图实例')
  } else {
    console.log('TimeSliderPanel 等待地图实例...')
  }
})
