  ...
  <section class="time-slider-panel" role="region" aria-label="轨迹查询面板">
    <div id="panel-content" class="panel-content" role="group" aria-label="查询参数设置">
      ...
      <div class="range-summary" role="status" aria-live="polite">
        <span><span aria-hidden="true">📊</span>区间：<b>{{ endIndex - startIndex + 1 }}</b> / {{ globalMaxIdx + 1 }} 点</span>
        <span><span aria-hidden="true">🕐</span>时长：<b>{{ formatDuration((endTimestamp||0) - (startTimestamp||0)) }}</b></span>
      </div>

      <!-- 🔘 操作按钮组 -->
      <div class="panel-actions" role="group" aria-label="查询操作">
        <button class="btn primary" @click="handleQuery" :disabled="loading" :aria-busy="loading">
          <span aria-hidden="true">{{ loading ? '🔄' : '🚀' }}</span>
          {{ loading ? '查询中...' : '刷新轨迹' }}
        </button>
        <button class="btn" @click="resetToDefault" :disabled="loading">🔄 重置</button>
        <button class="btn" @click="expandFull" :disabled="loading">📐 全量</button>
      </div>
    </div>
  </section>
</template>
...
watch(
  ...
  (newMap, oldMap) => {
    if (newMap && !oldMap) {
      console.log('🗺️ TimeSliderPanel: Map ready')
...
const onMapReady = (map) => {
  // Map 就绪后可自动查询（可选）
  if (props.mmsi && props.totalCount > 0) {
    emitQuery()
  }
}

onMounted(() => {
  // 兜底：如果 map 已就绪但未触发 watch
...
// 🔍 监听 props 变化，自动刷新（核心修复）
watch(
  () => [props.mmsi, props.totalCount],
  ([newMmsi, newTotal], [oldMmsi, oldTotal]) => {
    if (newMmsi && newTotal > 0 && (newMmsi !== oldMmsi || newTotal !== oldTotal)) {
      console.log(`🔄 TimeSliderPanel: params changed, mmsi=${newMmsi}, total=${newTotal}`)
      // 重置状态 + 自动查询
      resetToDefault()
      if (isMapReady.value) {
        emitQuery()
...
const handleQuery = async () => {
  if (!isMapReady.value || !mapInstance.value) {
    console.warn('⚠️ Map not ready, query cancelled')
    return
  }
  await emitQuery()
}

// 解构 useTimeSlider（确保 mapOp 是 computed）
const {
  startTimestamp, endTimestamp, startIndex, endIndex,
  timeStep, indexStep, loading, error,
  currentPoints, currentPointsCount,
  TIME_STEPS, INDEX_STEPS,
  isEditingTime, isEditingIndex,
  adjustTime, adjustIndex, updateTime, updateIndex,
  emitQuery, resetToDefault, expandFull,
  formatTimestamp, formatLocalDate, formatTimeOnly, formatDuration,
  globalMinIdx, globalMaxIdx, lastModifiedSource
} = useTimeSlider(mapOp, props.mmsi, props.totalCount)

// 🔓 暴露方法给父组件（仅用于简单委托，非必需）
defineExpose({
  emitQuery: handleQuery,
  resetToDefault,
  expandFull,
  exportTrajectory: () => mapOp.value?.exportToCSV?.(currentPoints.value), // 示例
  centerView: () => mapOp.value?.fitBounds?.(currentPoints.value), // 示例
  currentPointsCount,
  loading,
  isMapReady
})
...
.sync-indicator::after {
  ...
  border-radius: 50%;
  animation: ts-ring-pulse 2.5s infinite;
  opacity: 0;
}
