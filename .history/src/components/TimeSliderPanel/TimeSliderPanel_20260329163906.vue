...
const handleQuery = async () => {
  ...
  // ✅ 调用原始 emitQuery（来自 useTimeSlider）
  await emitQuery()
}

// ========== 🧩 解构 useTimeSlider Hook ==========
// ✅ 传入 computed 的 mapOp，确保 hook 内部能响应式获取最新 map
const {
  // 📊 状态
  startTimestamp,
  endTimestamp,
  ...
  updateTime,
  updateIndex,
  emitQuery,        // 📌 原始查询函数（被 handleQuery 包装）
  resetToDefault,
  expandFull,
...
defineExpose({
  ...
  // ✅ 暴露 map 状态（可选，方便父组件调试）
  isMapReady,
  mapInstance
})

// ========== 🎨 模板中使用的辅助方法 ==========
// ✅ 如果模板需要直接调用 query，绑定到 handleQuery
