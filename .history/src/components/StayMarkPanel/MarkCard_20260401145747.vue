...
import { 
  markTable, sortConfig, highlightedMarkId,
  getSortedMarks, toggleSort, submitMarksToDB,
  clearMarkPanel, highlightMark, updateMarkField,
  deleteMark, setCurrentMmsi
} from '@/components/MarkEditDialog/markFlow'
...
// ========== 监听MMSI ==========
watch(() => props.mmsi, (newMmsi) => {
  if (newMmsi) setCurrentMmsi(newMmsi)
}, { immediate: true })
...
const handleRowClick = (mark) => {
  ...
  emit('mark-highlight', mark)
}

const handleFieldUpdate = async (mark, field, value) => {
  await updateMarkField({ id: mark.id, field, value })
  emit('mark-update', { id: mark.id, field, value })
}

const handleSubmit = async () => {
  try {
    const result = await submitMarksToDB()
    if (result.success) {
      emit('submit-success', result)
      clearMarkPanel()  // ✅ 提交后清空面板
    }
  } catch (e) {
    emit('submit-error', e)
  }
...
const handleClear = () => {
  if (confirm('确定清空所有标记？')) clearMarkPanel()
}
...
const saveNoteEdit = async () => {
  if (noteEdit.value.targetMark) {
    await updateMarkField({ id: noteEdit.value.targetMark.id, field: 'note', value: noteEdit.value.content })
    emit('mark-update', { id: noteEdit.value.targetMark.id, field: 'note', value: noteEdit.value.content })
  }
  closeNoteEdit()
}
</script>
