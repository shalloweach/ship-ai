...
import { submitStayMarks } from '@/api/ship'
// ========== 📊 排序配置 ==========
 ...
 * 提交数据到数据库（接口定义）
 */
export const submitAllMarks = async (apiUrl = '/api/ship/marks') => {
  if (markTable.value.length === 0) {
    console.warn('没有标记可提交')
    return { success: false, message: '无数据可提交' }
  }

  try {
    // 准备提交数据：按开始时间升序，且只发送需要的字段
    const payload = sortedMarks.value.map(({ id, ...rest }) => rest)
    
    // 调用真实后端接口
    const response = await submitStayMarks(payload)  // 或直接使用 fetch / axios

    // 根据后端返回判断成功与否
    if (response.success) {
      // 提交成功，清空面板
      clearAllMarks()
      return { success: true, message: `成功提交 ${payload.length} 条记录` }
    } else {
