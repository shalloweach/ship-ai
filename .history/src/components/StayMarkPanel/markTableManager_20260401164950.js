...
export const submitMarks = async (marks = null, options = {}) => {
  const { mmsi } = options  // 获取传入的 mmsi
  const dataToSubmit = marks || markTable.value

  // 构建提交数据：移除内部 id 字段，并确保每条记录包含 mmsi
  const payload = dataToSubmit.map(({ id, ...rest }) => ({
    ...rest,
    mmsi: mmsi || rest.mmsi,  // 优先使用传入的 mmsi，若没有则保留原记录中的 mmsi（如果有）
  }))

  try {
    // 调用真实后端接口
    const response = await submitStayMarks(payload)
    ...
    if (response && response.success) {
      ...
      return {
        success: true,
        message: `成功提交 ${payload.length} 条记录`,
        data: response.data
      }
    } else {
    ...
    return { success: false, code: 'ERROR', message: error.message, error }
  }
}
