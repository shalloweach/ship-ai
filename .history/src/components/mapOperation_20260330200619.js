...
export default function useMapOperation(containerRef) {
  ...
  * 🗑️ 移除船舶图标
  ...
  const clearTrajectory = () => {
    if (trajectoryOverlay && mapInstance) {
      mapInstance.remove(trajectoryOverlay)
      trajectoryOverlay = null
    }
    if (pointMarkers.length) {
      pointMarkers.forEach(marker => {
        if (marker && mapInstance) mapInstance.remove(marker)
      })
      pointMarkers = []
    }
    // 注意：不清理 layers.polylines，由外部显式调用 removePolyline 清除
  }
  ...
  const parsePointToLngLat = (p) => {
    ...
    if (Array.isArray(p)) {
      // 自动识别 [lng, lat] 或 [lat, lng]
      if (p.length >= 2) {
        // 简单启发：如果第一个值在 -180~180 且第二个在 -90~90，则为 lng/lat，否则可能 lat/lng
        const isLngFirst = (p[0] >= -180 && p[0] <= 180) && (p[1] >= -90 && p[1] <= 90)
        return isLngFirst ? new AMap.LngLat(p[0], p[1]) : new AMap.LngLat(p[1], p[0])
      }
    }
    console.warn('无法解析点:', p)
    return null
  }
