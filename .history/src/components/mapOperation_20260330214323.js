  ...
  const switchBaseLayer = (type) => {
    ...
    switch (type) {
      case LAYER_TYPES.SATELLITE:
        if (!layers.customLayers[LAYER_TYPES.SATELLITE]) {
          layers.customLayers[LAYER_TYPES.SATELLITE] = new AMap.TileLayer.Satellite()
        }
        targetLayer = layers.customLayers[LAYER_TYPES.SATELLITE]
        break

      case LAYER_TYPES.ROADNET:
        if (!layers.customLayers[LAYER_TYPES.ROADNET]) {
          // 路网图需要高德地图插件支持（AMap.TileLayer.RoadNet 需引入）
          if (typeof AMap.TileLayer.RoadNet === 'function') {
            layers.customLayers[LAYER_TYPES.ROADNET] = new AMap.TileLayer.RoadNet()
          } else {
            console.warn('当前高德版本不支持路网图，请确保已引入相关插件')
            return
          }
        }
        targetLayer = layers.customLayers[LAYER_TYPES.ROADNET]
        break

      case LAYER_TYPES.TERRAIN:
        if (!layers.customLayers[LAYER_TYPES.TERRAIN]) {
          if (typeof AMap.TileLayer.Terrain === 'function') {
            layers.customLayers[LAYER_TYPES.TERRAIN] = new AMap.TileLayer.Terrain()
          } else {
            console.warn('当前高德版本不支持地形图，请确保已引入相关插件')
            return
          }
        }
        targetLayer = layers.customLayers[LAYER_TYPES.TERRAIN]
        break

      default:
        console.warn(`未知图层类型: ${type}，请使用预定义常量 LAYER_TYPES`)
        return
    }

    if (targetLayer) {
      targetLayer.setMap(mapInstance)
      layers.currentCustomLayer = targetLayer
      console.log(`🛰️ 已切换为 ${type} 地图`)
    }
  }
