...
export default function useMapOperation(containerRef) {
  ...
  // 切换图层
  const switchBaseLayer = (type.value) => {
    console.log(type)
    // 标准地图：无需额外图层，直接返回
    if (type === 'standard') {
    ...
    switch (type) {
      ...
      case 'roadnet':
        ...
        targetLayer = layers.customLayers['roadnet']
        break

      case 'terrain':
        if (!layers.customLayers['terrain']) {
          if (typeof AMap.TileLayer.Terrain === 'function') {
            layers.customLayers['terrain'] = new AMap.TileLayer.Terrain()
          } else {
            console.warn('当前高德版本不支持地形图，请确保已引入相关插件')
            return
          }
        }
        targetLayer = layers.customLayers['terrain']
        break
