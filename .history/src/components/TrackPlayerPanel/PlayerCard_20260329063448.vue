...
<template>
  <fieldset class="control-card player-card">
    ...
    <div class="player-controls">
      <!-- ✅ 移除 :disabled，直接调用 player 方法 -->
      <button class="btn-play" @click="player.togglePlay()">
        <span aria-hidden="true">{{ player.isPlaying.value ? '⏸️' : '▶️' }}</span>
        {{ player.isPlaying.value ? '暂停' : '播放' }}
      </button>
      
      <button class="btn-stop" @click="player.stopPlay()">
        <span aria-hidden="true">⏹️</span> 停止
      </button>
      
      <!-- ⚡ 倍速选择 -->
      <select v-model="speed" 
              @change="player.setSpeed(Number($event.target.value))" 
              class="speed-select"
              aria-label="播放速度">
    ...
    <div class="player-start">
      <button class="btn-start" @click="player.startPlay(0)" 
              title="从头开始播放">
        <span aria-hidden="true">⏮️</span> 从头播放
      </button>
      <button class="btn-start" @click="player.startPlay()" 
              title="从当前位置继续播放">
