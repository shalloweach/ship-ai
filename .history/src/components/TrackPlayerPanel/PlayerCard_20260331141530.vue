...
import {formatTime, formatNavigationStatus} from '@/components/utils'
...
// 起始索引变更处理
const handleStartIndexChange = () => {
  const max = Math.max(0, props.totalCount - 1)
  let value = localStartIndex.value
  if (value < 0) value = 0
  if (value > max) value = max
  localStartIndex.value = value
  setIndex(value)
}

// 监听 totalCount 变化时重置起始索引
watch(() => props.totalCount, (newVal) => {
  if (newVal <= 0) {
    localStartIndex.value = 0
  } else if (localStartIndex.value >= newVal) {
    localStartIndex.value = Math.max(0, newVal - 1)
  }
})
