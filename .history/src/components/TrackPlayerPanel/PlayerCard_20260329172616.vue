<template>
  <fieldset class="control-card player-card">
    ...
    <div class="player-actions">
      <button class="btn btn-play" @click="handleStartPause"
    ...
    <div class="player-progress">
      ...
      <div class="progress-info">
        <span class="progress-index">{{ player.currentIndex.value + 1 }} / {{ totalCount }} 点</span>
...
// 🗺️ 统一使用 inject 获取地图操作（替代 props.mapOp）
const mapOpRef = inject('mapOp')
const mapOp = computed(() => mapOpRef?.value)

// 响应式包装（useTrackPlayer 需要 ref）
const mmsiRef = ref(props.mmsi)
const totalCountRef = ref(props.totalCount)

// 同步 props → ref
watch(() => props.mmsi, (val) => { mmsiRef.value = val })
watch(() => props.totalCount, (val) => { totalCountRef.value = val })
...
watch(
  ...
  ([newMmsi, newTotal], [oldMmsi, oldTotal]) => {
    if (newMmsi && newTotal > 0 && (newMmsi !== oldMmsi || newTotal !== oldTotal)) {
      console.log(`🔄 PlayerCard: params changed, reset player`)
      player.stopPlay()
      localStartIndex.value = 0
      // 可选：自动开始播放
      // if (mapOp.value?.map) player.startPlay(0)
    }
  }
)

// 初始化播放器（传入 mapOp）
const player = useTrackPlayer(mmsiRef, totalCountRef, mapOp)
