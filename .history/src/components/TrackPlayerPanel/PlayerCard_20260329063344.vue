...
<template>
  <fieldset class="control-card player-card">
    ...
    <div class="player-controls">
      <!-- ✅ 移除 :disabled="!player.canPlay.value" -->
      <button class="btn-play" @click="handleTogglePlay">
        <span aria-hidden="true">{{ player.isPlaying.value ? '⏸️' : '▶️' }}</span>
        {{ player.isPlaying.value ? '暂停' : '播放' }}
      </button>
      
      <button class="btn-stop" @click="handleStopPlay">
    ...
    <div class="player-start">
      <button class="btn-start" @click="handleStartFromBeginning" 
              title="从头开始播放">
        <span aria-hidden="true">⏮️</span> 从头播放
      </button>
      <button class="btn-start" @click="handleContinuePlay" 
              title="从当前位置继续播放">
    ...
    <!-- ⚠️ 条件不满足时的提示 -->
    <div v-if="!canPlayInternal" class="hint-text" role="note">
      <small>💡 点击按钮将尝试加载轨迹数据</small>
    </div>
  </fieldset>
</template>
