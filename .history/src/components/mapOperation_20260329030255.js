...
export class MapOperation {
  ...
  /**
   * 绘制停留标记区域
   * @param {Object} marker - 停留标记数据
   */
  drawStayMarker(marker) {
    if (!this.map || !marker?.startPoint || !marker?.endPoint) return null
    
    const { id, startPoint, endPoint, stayType = '停留', port, startTime, endTime } = marker
    
    // 样式配置
    const styles = {
      '锚泊': { color: '#f59e0b', fillOpacity: 0.15, icon: '⚓' },
      '靠泊': { color: '#10b981', fillOpacity: 0.15, icon: '🏭' },
      '其他': { color: '#6b7280', fillOpacity: 0.1, icon: '📍' }
    }
    const style = styles[stayType] || styles['其他']
    ...
    const iconMarker = L.marker([startPoint.lat, startPoint.lng], { icon }).addTo(this.map)
    ...
    const popupContent = `
      ...
      ⏱️ ${this._formatDuration(duration * 1000)}
    `
    rect.bindPopup(popupContent)
    iconMarker.bindPopup(popupContent)
    
    // 保存引用
    this.layers.stayMarkers.set(id, { rect, iconMarker })
   ...
   */
  removeStayMarker(id) {
    const layers = this.layers.stayMarkers.get(id)
    if (!layers || !this.map) return false
    
    if (layers.rect && this.map.hasLayer(layers.rect)) {
      this.map.removeLayer(layers.rect)
    }
    if (layers.iconMarker && this.map.hasLayer(layers.iconMarker)) {
      this.map.removeLayer(layers.iconMarker)
    }
   ...
   */
  clearStayMarkers() {
    this.layers.stayMarkers.forEach(({ rect, iconMarker }) => {
      if (this.map) {
        if (rect && this.map.hasLayer(rect)) this.map.removeLayer(rect)
        if (iconMarker && this.map.hasLayer(iconMarker)) this.map.removeLayer(iconMarker)
      }
    })
