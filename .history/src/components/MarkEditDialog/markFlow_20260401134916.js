...
export const markFlow = ref({
    ...
    onPointSelected: null            // 🎯 终点选择回调（由 pickPointFromMap 注册）
  })
...
export const startMarkFlow = async (mapState, points, startIdx) => {
    try {
      ...
      const endPointSelected = (endIdx, endPoint) => {
    ...
    return new Promise((resolve) => {
      ...
      const onPointSelected = (point, lnglat) => {
        ...
        const idx = trajectoryPoints.findIndex(p => {
          ...
          // 使用 0.0001° ≈ 11m 的容错阈值
          return Math.abs(pLat - clickedLat) < 0.0001 &&
                 Math.abs(pLon - clickedLng) < 0.0001
        })
      
        console.log(`🎯 点击匹配: idx=${idx}, point=`, point)
