...
import {showSuccessToast} from '@/components/utils'
// 响应式状态
...
export const clearRubberBand = (mapState) => {
  ...
  export const createMarkData = (startPoint, endPoint) => {
    const startTs = startPoint.utc_time 
    const endTs = endPoint.utc_time
   ...
   */
export const onEditOpen = (markData, segment) => {
  let draughts = segment.map(point => point.draught)
  let draughts = segment[0]
  ...
  export const startMarkFlow = async (mapState, points, startIdx) => {
    try {
      ...
      const endPointSelected = (endIdx) => {
        ...
        const { promise, close } = onEditOpen(tempMark, segment)
