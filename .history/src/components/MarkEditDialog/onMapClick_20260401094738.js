// src\components\MarkEditDialog\onMapClick.js
处理点击标记后的流畅。1.再选一个点 2.渲染两点间轨迹 3.把这两点整理成一条记录传给 下一步打开openMarkCard.js来编辑这个窗口 手动填充其他数据
最后上传到表格
...
export const startMarkFlow = (mapState, points, startIdx) => {
    ...
    tempMark = onEditOpen(tempMark, { isNew: true })
    4.关闭高亮
    
    5.提交数据
    saveMarkToTable(Marker)
}
