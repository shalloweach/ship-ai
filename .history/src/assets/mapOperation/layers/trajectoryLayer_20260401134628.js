...
import { startMarkFlow, markFlow } from '@/components/MarkEditDialog/markFlow'
/**
 * 渲染船舶轨迹
 */
export const renderTrajectory = (mapState, points, options = {}, popupApi) => {
  ...
  if (showPoints && path.length <= 5000) {
    mapState.pointMarkers = path.map((lnglat, idx) => {
      ...
      marker.on('click', () => {
        // 🔥 如果当前处于标记流程（选择终点阶段），则使用回调处理
        if (markFlow.value.mode === 'selecting-end' && markFlow.value.onPointSelected) {
          // ✅ 记录终点索引
          markFlow.value.endPoint = idx
          
          // ✅ 调用已注册的回调函数处理终点选择
          // 注意：onPointSelected 签名是 (point, lnglat)
          const point = points[idx]
          const lnglat = { 
            lng: point.lon || point.lng, 
            lat: point.lat,
            // 兼容 AMap.LngLat 接口（如果回调内用了 getLng/getLat）
            getLng: () => point.lon || point.lng,
            getLat: () => point.lat
          }
          
          // 执行回调，让 pickPointFromMap 的 Promise  resolve
          markFlow.value.onPointSelected(point, lnglat)
          
          return   // 🚫 不显示信息面板，流程由 markFlow 接管
        }
        
        // 否则，正常显示信息面板（仅在空闲模式）
        if (markFlow.value.mode !== 'idle') return
      
        popupApi?.showPopup?.(marker, points[idx], {
          onMarkClick: (point) => {
            startMarkFlow(mapState, points, idx)
            popupApi?.closePopup?.()
          }
        })
      })
