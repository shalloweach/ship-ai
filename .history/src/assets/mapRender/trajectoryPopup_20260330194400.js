...
const formatTime = (timestamp) => {
  ...
  export const buildPopupContent = (point) => {
    ...
    // 📌 获取字段（单一路径）
    {"course":149.4,"destination":"","draught":0.0,"idx":5,"lat":22.437446666666666,"lon":113.88231166666667,"navigationstatus":"0.0","speed":0.65,"utc_time":1672549379}
    ...
    // 💡 各字段显示值
    const v = {
      time: utc_time ? formatTime(utc_time) : '-',
      coord: (lat != null && lon != null) 
        ? `${formatCoord(lat)}, ${formatCoord(lon)}` : '-',
      speed: speed != null ? formatValue(speed, ' kn', 1) : '-',
      course: course != null ? `${Math.round(pointcourse)}°` : '-',
      rot: rot != null ? formatValue(rot, '°/min', 1) : '-',
      draught: draught != null ? formatValue(draught, ' m', 2) : '-',
      destination: destination || '-',
      eta: eta ? formatTime(eta) : '-',
      status: formatNavStatus(navStatus)
    }
