...
<template>
  ...
  const initDifyChatbot = () => {
    ...
    window.difyChatbotConfig = {
      token: 'qylJxmAStyBnzDZc',
      baseUrl: 'http://106.15.176.173',
      inputs: {
        mmsi: mmsi.value || ''
      },
      systemVariables: {
    ...
    script.src = 'http://106.15.176.173/embed.min.js'
    script.id = 'qylJxmAStyBnzDZc'
    script.defer = true
  ...
  const searchShip = async () => {
    ...
    try {
      ...
      if (window.difyChatbotConfig) {
        window.difyChatbotConfig.inputs = {
          mmsi: mmsi.value,
          shipName: shipInfo.value?.name || '',
          totalBerthingHours: itineraryData.value?.summaryStats?.totalBerthingHours || 0
        }
      }
